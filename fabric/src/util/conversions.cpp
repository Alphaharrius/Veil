...
int8 veil::util::to_signed(uint8 val) {
    ...
    assert((val <= MAX_INT8 && veil::implementation_fault("Integer overflow.", VeilGetLineInfo)));
    return (int8) val;
}

uint8 veil::util::to_unsigned(int8 val) {
    assert((val >= 0 && veil::implementation_fault("Negative value to unsigned.", VeilGetLineInfo)));
    return (uint8) val;
}

int16 veil::util::to_signed(uint16 val) {
    static const uint16 MAX_INT16 = (1U << 15) - 1;
    assert((val <= MAX_INT16 && veil::implementation_fault("Integer overflow.", VeilGetLineInfo)));
    return (int16) val;
}
...
int32 veil::util::to_signed(uint32 val) {
    ...
    assert((val <= MAX_INT32 && veil::implementation_fault("Integer overflow.", VeilGetLineInfo)));
    return (int32) val;
}
...
int64 veil::util::to_signed(uint64 val) {
    ...
    assert((val <= MAX_INT64 && veil::implementation_fault("Integer overflow.", VeilGetLineInfo)));
    return (int64) val;
}
